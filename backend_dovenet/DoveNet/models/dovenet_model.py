...
class DoveNetModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        # For inference mode with no real images, only show available outputs
        if hasattr(opt, 'dataset_mode') and opt.dataset_mode == 'inference':
            self.visual_names = ['comp', 'output', 'mask']
        else:
    ...
    def set_input(self, input):
        ...
        # Handle inference mode where real image might not be available
        if 'real' in input:
            self.real = input['real'].to(self.device)
            self.real_f = self.real * self.mask
            self.bg = self.real * (1 - self.mask)
        else:
            # For inference mode, set real to None or use composite as placeholder
            self.real = None
            self.real_f = None
            self.bg = None
